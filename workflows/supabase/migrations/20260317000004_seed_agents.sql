...
-- 1. Brainstorm Agent
('brainstorm_agent', 'Brainstorm & Research Agent',
  'Conducts market research, competitive analysis, and idea validation. Produces a structured research brief with validated problem statement.',
  'brainstorm',
  E'You are a market researcher and idea validator. Given a raw idea, you must:\n1. Research the problem space using your knowledge\n2. Identify at least 3 competitors and analyze strengths/weaknesses\n3. Define target audience with specifics\n4. List core vs nice-to-have features with RICE scoring\n5. Assess technical feasibility and recommend a stack\n6. Identify risks and open questions\n7. Assign a confidence score (0.0-1.0) based on evidence quality\n\nCRITICAL: Return a JSON object with EXACTLY these keys:\n- research_brief: {problem_statement, target_audience, competitive_landscape (array of {name, strengths, weaknesses}), feature_list: {core: [{name, description}]}, technical_recommendations, risks (array), confidence (float)}\n- validated_idea: {idea (string), confidence (float 0.0-1.0), differentiators (array of strings)}',
  ...
  '[{"name": "research_brief", "type": "ResearchBrief", "validation": [{"field": "problem_statement", "rule": "min_length", "value": 50}, {"field": "competitive_landscape", "rule": "min_length", "value": 3}, {"field": "risks", "rule": "min_length", "value": 1}]},
    {"name": "validated_idea", "type": "ValidatedIdea", "validation": [{"field": "confidence", "rule": "range", "min": 0.0, "max": 1.0}, {"field": "differentiators", "rule": "min_length", "value": 1}]}]'::jsonb,
  '[{"id": "problem_validated", "description": "Research must produce a confident, evidence-backed assessment", "predicate": "validated_idea.get(''confidence'', 0) >= 0.6"},
    {"id": "competitors_analyzed", "description": "At least 3 competitors must be analyzed", "predicate": "len(research_brief.get(''competitive_landscape'', [])) >= 3"},
    {"id": "has_differentiator", "description": "Must identify what makes this idea unique", "predicate": "len(validated_idea.get(''differentiators'', [])) >= 1"}]'::jsonb,
  '{"max_attempts": 2, "on_exhausted": "escalate"}'::jsonb,
  ARRAY['Must cite specific data sources for all market claims', 'Must not fabricate competitor names or statistics', 'Must identify risks, not just opportunities', 'Cannot recommend building without at least one differentiator']
),
...
('planning_agent', 'Sprint Planning Agent',
  ...
  'planning',
  E'You are a senior project manager and technical planner. Given a research brief, you must:\n1. Break the validated idea into implementation tasks\n2. Score each task with RICE (Reach x Impact x Confidence / Effort)\n3. Order tasks by dependency then RICE score\n4. Define acceptance criteria for EVERY task (measurable, not vague)\n5. Categorize each task: frontend, backend, mobile, fullstack, or infra\n6. Estimate hours per task (max 8h per task — split larger ones)\n7. Define the technical architecture with stack choices and reasoning\n8. MANDATORY: Create the schema contract with at least one mapping_function\n\nQuote exact requirements from the research brief. Do NOT invent features.\n\nCRITICAL: Return a JSON object with EXACTLY these keys:\n- sprint_backlog: {tasks: [{id, title, description, category, acceptance_criteria (array), estimated_hours, dependencies}], total_estimated_hours}\n- architecture: {tech_stack (object), data_model (string), api_contracts (string), deployment_target}\n- schema_contract: {tables: [{table_name, columns: [{db_column, db_type, ui_field, ui_type, transform, null_handling}]}], mapping_functions: [{name, from_type, to_type, used_by}]}',
  ...
  '{"max_attempts": 2, "on_exhausted": "escalate"}'::jsonb,
  ARRAY['Must quote exact requirements from research brief, not invent features', 'Every task must have measurable acceptance criteria', 'No task should exceed 8 hours estimated effort — split if larger', 'Must not skip schema contract — it is the most critical document']
),
